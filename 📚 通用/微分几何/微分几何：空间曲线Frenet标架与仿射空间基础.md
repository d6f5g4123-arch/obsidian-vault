...
### 1.1 Frenet标架定义
Frenet标架是描述空间曲线上某点局部性质的正交基，是曲线微分几何的重要工具。

- **设定**：设 $r:[a, b]\to \mathbb{R}^3$ 是一条可微 $C^2$ 曲线 $C$ 的==弧长参数化==。
- **定义**：在点 $r(s)$ 处，由切向量、主法向量、副法向量构成的正交标架称为Frenet标架。
- **常用记号**：
  - $\boldsymbol{\alpha}(s)$：切向量
  - $\boldsymbol{\beta}(s)$：主法向量
  - $\boldsymbol{\gamma}(s)$：副法向量

==考试重点：Frenet标架下的切向量、法向量、副法向量的定义及其几何意义==

### 1.2 再参数化下的曲率表达式（定理2.1）
- **前提**：设 $r$ 为弧长参数化，$q$ 为正则再参数化，参数变换为 $S: (c, d) \to (a, b),\ t \to s(t)$，且 $q(t) = r(s(t))$。
- **曲率表达式**：
  $$
...
- **导数关系**：
  ...
  $$
- **推导关键**：
  1. 由于 $|\dot{r}(s)| = 1$，有 $\dfrac{ds}{dt}=|\dot{q}(t)|_2$。
  2. 弧长参数化下 $\dot{r}(s) \perp \ddot{r}(s)$。
  3. 可推出 $\dot{r}(s)\parallel\dot{q}(t)$，且
     $$
...
### 1.4 挠率
- 设 $r:(a, b)\to \mathbb{R}^3$，挠率用于描述曲线偏离平面曲线的程度，定义待补充。
...
**证明思路**：利用坐标变换映射 $\varphi_{\mathcal{A}'} \circ \varphi_{\mathcal{A}}^{-1}$ 是连续的双射（同胚），其逆也连续，从而保证连续性在不同坐标系下的一致性。
...
## 四、 易混淆点与总结
1.  **Frenet标架**：基于**弧长参数化**定义，是曲线局部性质的“活动标架”。
2.  **曲率计算**：在非弧长参数化下，必须使用定理2.1的公式 $k(s) = \frac{\left| \ddot{q}^{\perp}(t) \right|_2}{\left| \dot{q}(t) \right|_2^2}$ 进行计算。
3.  **平面法向量**：法平面、从切平面、法切平面的法向量分别是Frenet标架中的切向量、主法向量、副法向量，注意区分。
4.  **仿射空间 vs 向量空间**：仿射空间没有“原点”的概念，点与点之间是向量，点本身不是向量。
5.  **坐标变换**：仿射坐标变换包含原点的平移 ($a^i$) 和基的线性变换 ($T_i^{\, j}$) 两部分。
6.  **几何量**：是仿射几何研究的核心，即不依赖于坐标系选取的、描述几何对象内在性质的量。
