...
设 $r:[a, b]\to \mathbb{R}^3$ 是一条可微 $C^2$ 曲线 $C$ 的**弧长参数化**。在点 $r(s)$ 处，定义：
...
由 $\{\boldsymbol{\alpha}(s), \boldsymbol{\beta}(s), \boldsymbol{\gamma}(s)\}$ 构成的正交基称为该点的 **Frenet标架**。
...
由于 $|\dot{r}(s)| = 1$，有 $\dfrac{ds}{dt}=|\dot{q}(t)|_2$，且 $\dot{r}(s) \perp \ddot{r}(s)$。由此可得 $\dot{r}(s)\parallel\dot{q}(t)$，并有：
$$
...
设 $r:(a, b)\to \mathbb{R}^3$ 为弧长参数化的正则曲线。**挠率** $\tau(s)$ 度量曲线偏离平面曲线的程度，定义为：
$$
\dot{\boldsymbol{\gamma}}(s) = -\tau(s) \boldsymbol{\beta}(s)
$$
或等价地，$\tau(s) = -\dot{\boldsymbol{\gamma}}(s) \cdot \boldsymbol{\beta}(s)$。
...
**命题**：设点 $A$ 在坐标系 $\mathcal{A}$ 下的坐标为 $(x^{1}, \dots, x^{n})$，在坐标系 $\mathcal{A}'$ 下的坐标为 $(y^{1}, \dots, y^{n})$，则变换关系为：
$$
...
**几何量**是仅依赖于几何对象本身，而不依赖于坐标系选取的量。例如，曲线的弧长、曲率、挠率，以及点、线、面之间的夹角和距离等，都是几何量。
...
**证明思路**：利用坐标变换映射 $\varphi_{\mathcal{A}'} \circ \varphi_{\mathcal{A}}^{-1}$ 是连续的双射（同胚）这一性质。

## 三、关联知识

- [[2026-03-09-拓扑学基础概念]]
- 微分几何中，Frenet标架理论是研究曲线局部微分性质的基础，而仿射空间及其坐标理论则为在更一般的背景下描述几何对象提供了框架。两者结合，可以更深入地理解几何对象的**内蕴性质**（不依赖于坐标选取）与**外在描述**（依赖于坐标选取）之间的关系。

## 四、易混淆点
1.  **Frenet标架中的“法向量”**：通常指**主法向量** $\boldsymbol{\beta}(s)$，它指向曲线弯曲的方向。而“法平面”的法向量是**切向量** $\boldsymbol{\alpha}(s)$，切勿混淆。
2.  **仿射空间与向量空间**：仿射空间没有天然的“原点”，点与点之间是向量，但点本身不是向量。向量空间有固定的零元。
3.  **几何量与坐标量**：曲率、挠率是几何量；点的具体坐标 $(x, y, z)$ 不是几何量，依赖于坐标系的选取。
