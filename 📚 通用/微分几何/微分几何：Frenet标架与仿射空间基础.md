...
- **切向量** $\boldsymbol{\alpha}(s)$：沿曲线切线方向
- **主法向量** $\boldsymbol{\beta}(s)$：指向曲线弯曲方向
- **副法向量** $\boldsymbol{\gamma}(s)$：由 $\boldsymbol{\alpha}(s) \times \boldsymbol{\beta}(s)$ 给出
...
其中 $\ddot{q}^{\perp}(t) = \left(\frac{ds}{dt}\right)^2 \ddot{r}(s)$，最终可推出：
$$
...
### 1.4 挠率
设 $r:(a, b)\to \mathbb{R}^3$，**挠率**是描述曲线偏离平面曲线程度的量，与副法向量的变化率有关。
...
设点 $A$ 在坐标系 $\mathcal{A}$ 下的坐标为 $(x^{1}, \dots, x^{n})$，在坐标系 $\mathcal{A}'$ 下的坐标为 $(y^{1}, \dots, y^{n})$，则变换关系为：
$$
