...
- **切向量 (Tangent Vector)** $\boldsymbol{\alpha}(s)$：指向曲线在该点的瞬时运动方向。
- **主法向量 (Normal Vector)** $\boldsymbol{\beta}(s)$：指向曲线在该点弯曲的方向（指向曲率中心）。
- **副法向量 (Binormal Vector)** $\boldsymbol{\gamma}(s)$：由切向量与主法向量叉积得到，$\boldsymbol{\gamma}(s) = \boldsymbol{\alpha}(s) \times \boldsymbol{\beta}(s)$。

==考试重点：Frenet标架下的切向量、法向量、副法向量的定义及其几何意义==

### 1.2 定义与假设
设 $r: [a, b] \to \mathbb{R}^3$ 是一条可微 $C^2$ 曲线 $C$ 的**弧长参数化**。在点 $r(s)$ 处，由切向量、主法向量、副法向量构成的标架称为该点的 Frenet 标架。
...
由于 $|\dot{r}(s)| = 1$，有 $\dfrac{ds}{dt}=|\dot{q}(t)|_2$，且 $\dot{r}(s) \perp \ddot{r}(s)$。由此可推出 $\dot{r}(s)\parallel\dot{q}(t)$，并有：
$$
...
### 1.5 挠率
设 $r:(a, b)\to \mathbb{R}^3$ 为弧长参数曲线，**挠率** $\tau(s)$ 描述了曲线偏离密切平面的程度，定义为副法向量 $\boldsymbol{\gamma}(s)$ 关于弧长的变化率：
$$
