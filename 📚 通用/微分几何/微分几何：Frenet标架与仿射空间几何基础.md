...
## 一、 空间曲线的Frenet标架与局部性质

### 1.1 Frenet标架定义
Frenet标架是描述空间曲线上某点局部性质的正交基，是曲线微分几何的重要工具。

- 设 $r:[a, b]\to \mathbb{R}^3$ 是一条可微 $C^2$ 曲线 $C$ 的**弧长参数化**。
- 称 $r(s)$ 处的切向量、主法向量、副法向量为该点的 **Frenet标架**。
- 常以 $\boldsymbol{\alpha}(s)$、$\boldsymbol{\beta}(s)$、$\boldsymbol{\gamma}(s)$ 分别表示切向量、主法向量、副法向量。
...
### 1.4 挠率
- 设 $r:(a, b)\to \mathbb{R}^3$，**挠率**是描述曲线偏离平面曲线程度的量，与副法向量 $\boldsymbol{\gamma}(s)$ 的变化率有关。
...
## 三、 关联与总结
- **Frenet标架**是微分几何中研究**曲线局部性质**的核心工具，与**曲率**、**挠率**紧密相关。
- **仿射空间**为研究几何对象（点、线、面）及其坐标表示提供了抽象的框架，其上的**拓扑**和**函数**连续性定义与坐标系选取无关。
- 两者共同构成了微分几何和经典几何学的基础。
