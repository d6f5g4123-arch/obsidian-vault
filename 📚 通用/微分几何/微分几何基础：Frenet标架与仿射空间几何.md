---
title: "微分几何基础：Frenet标架与仿射空间几何"
date: 2026-03-25
subject: 其他
subtopic: 微分几何
tags: [微分几何, 仿射空间, Frenet标架, 曲率, 拓扑]
consolidated: true
---

# 微分几何基础：Frenet标架与仿射空间几何
...
称 $r(s)$ 处的切向量、主法向量、副法向量为该点的 Frenet 标架。常以 $\boldsymbol{\alpha}(s)$、$\boldsymbol{\beta}(s)$、$\boldsymbol{\gamma}(s)$ 分别表示切向量、主法向量、副法向量。
...
- 以 $\boldsymbol{\gamma}(s)$（副法向量）为法向量：**密切平面**

==考试重点：三种平面的法向量分别是谁==

### 2.4 挠率
设 $r:(a, b)\to \mathbb{R}^3$ 为弧长参数化的正则曲线，其挠率 $\tau(s)$ 定义为副法向量 $\boldsymbol{\gamma}(s)$ 的变化率在切向量方向上的分量，或主法向量 $\boldsymbol{\beta}(s)$ 的变化率在副法向量方向上的分量，即 $\dot{\boldsymbol{\gamma}}(s) = -\tau(s) \boldsymbol{\beta}(s)$。挠率描述了曲线偏离平面曲线的程度。
